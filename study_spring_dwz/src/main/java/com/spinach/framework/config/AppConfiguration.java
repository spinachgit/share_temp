package com.spinach.framework.config;
...
import com.spinach.business.website.util.WebSiteUtils;
...
public class AppConfiguration {
	...
	private Map<String, com.spinach.business.website.Template> webTemplateMap = null;
	...
	private void fetchWebsiteTemplate() {
		...
		Object oNames = this.conf.getProperty("web-template-list.template.name");
		if (oNames == null) {
			return;
		}
		this.webTemplateMap = new TreeMap<String, com.spinach.business.website.Template>();
		...
		for (String templateName : templateNames) {
			com.spinach.business.website.Template template = null;

			String templateLabel = this.conf.getString("web-template-list.template(" + i + ").label");
			String description = this.conf.getString("web-template-list.template(" + i + ").description");

			String prePicBase = this.conf.getString("web-template-list.template(" + i + ").pre-pic-base");
			String prePicExt = this.conf.getString("web-template-list.template(" + i + ").pre-pic-ext");
			...
			Object oLayoutNames = this.conf.getProperty("web-template-list.template(" + i + ").layout-list.layout.name");
			...
			for (String layoutName : layoutNames) {
				String css = this.conf.getString("web-template-list.template(" + i + ").layout-list.layout(" + j + ").css");
				String indexStr = this.conf.getString("web-template-list.template(" + i + ").layout-list.layout(" + j + ").index");
				String desc = this.conf.getString("web-template-list.template(" + i + ").layout-list.layout(" + j + ").description"); //
			...
			Object oThemeNames = this.conf.getProperty("web-template-list.template(" + i + ").theme-list.theme.name");
	...
	private void fetchMailTemplateSetting() {
		...
		for (String name : names) {
			String subject = this.conf.getString("vm-template(" + i + ").subject");
	...
	public MailTemplate getMailTemplate(String name) {
		...
		if (template == null || template.getBody() == null) {
			template = this.mailTemplateMap.get(MailTemplateKey.defaultVm);
		}
	...
	public Map<String, com.spinach.business.website.Template> getWebTemplateMap() {
	...
	public String getWebServerName() {
		return this.conf.getString("app.server.name");
	}
